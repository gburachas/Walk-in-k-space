...
int blochsim(double *b1real, double *b1imag, 
		...
		double *mx, double *my, double *mz, int mode, double *mrsigr, double *mrsigi)
...
double gammadz;
double rotmat[9];
double amat[9], bvec[3];	/* cumulative A and B propagation matrix and vector */
double a0mat[9], b0vec[3];	/* stepwise   A and B propagation matrix and vector */
...
eyemat(a0mat); 		/* A is the identity matrix.	*/
...
for (tcount = 0; tcount < ntime; tcount++)
    ...
    #ifdef GTB_DEBUG
    printf("mrsig before = [%6.3f  %6.3f] \n",*mrsigr,*mrsigi);    
    #endif
    
    
    multmats(decmat,rotmat,a0mat);

	multmatvec(a0mat,mnow,mevolved);
	addvecs(mevolved,decvec,mevolved);
    ...
    *mrsigi+= mevolved[1];  
    ...
    #ifdef GTB_DEBUG
    printf("mevolve = [%6.3f  %6.3f  %6.3f ] \n",mevolve[0],mevolve[1],mevolve[2]);
    printf("mrsig after = [%6.3f  %6.3f] \n",*mrsigr,*mrsigi);    
    #endif
...
int blochsimfz(double *b1real, double *b1imag, double *xgrad, double *ygrad, double *zgrad, 
        ...
        double *mrsigreal, double *mrsigimag,
...
for (fcount=0; fcount < nfreq; fcount++)
	...
	blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			...
			*dzptr++, mx++, my++, mz++, mode, mrsigreal, mrsigimag);
		
    
    #ifdef GTB_DEBUG
     printf("mrsig first out = [%6.3f  %6.3f] \n",*mrsigr,*mrsigi);    
     printf("mrsig last  out = [%6.3f  %6.3f] \n\n",mrsigreal[ntime-1],mrsigimag[ntime-1]);    
    #endif
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
	...
	/* [mx,my,mz,mrsig] = bloch(b1,gradxyz,dt,t1,t2,df,dx,dy,dz,mode,mx,my,mz) */
{
...
/* ====================== Allocation of complex SIGNAL array =========================*/



plhs[3] = mxCreateDoubleMatrix(ntime,1,mxCOMPLEX);		


mrsigr = mxGetPr(plhs[3]);
mrsigi = mxGetPi(plhs[3]);
for (count =0; count < ntime; count++)
		{
		mrsigr[count] = 0;
		mrsigi[count] = 0;
...
#ifdef DEBUG
  printf("%d SIGNAL points created.\n",ntime);
#endif
...
blochsimfz(b1r,b1i,gx,gy,gz,tp,ntime,t1,t2,df,nf,dx,dy,dz,npos,mrsigr,mrsigi,mx,my,mz,(int)(md));
